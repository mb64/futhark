...
readHexRealLit s =
  ...
  let comps = T.split (`elem` ".pP") num in
  case comps of
    [i, f, p] ->
        let intPart = fromIntegral $ fst $ fromRight $ T.hexadecimal i
            fracPart = fromIntegral $ fst $ fromRight $ T.hexadecimal f
            exponent = fromIntegral $ fst $ fromRight $ T.signed T.decimal p
